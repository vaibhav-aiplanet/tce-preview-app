...
import { useSetCurrentGrade, useSetCurrentSubject, useSetCurrentSubtopic } from "~/store"
...
export default function Home() {
    ...
    const setCurrSubjectName = useSetCurrentSubject()
    const setCurrSubtopicName = useSetCurrentSubtopic()
    ...
    const loadAssetIds = useCallback(async (filePath: string) => {
        ...
        try {
            ...
            setCurrSubjectName(data.subject_name)
            setCurrSubtopicName(data.subtopic_name)
            setPage(0);
        } catch {
            setBatchAssetIds([]);
            setPage(0);
    ...
    useEffect(() => {
        if (initialBookRef.current) {
            ...
            setCurrentGrade(selectedGrade.length > 0 ? selectedGrade : null)
        }
